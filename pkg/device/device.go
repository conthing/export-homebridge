...
var ErrMacInvaild  = errors.New("ErrMacInvaild")
...
func createConfigData(accessaries []Accessary) (configdata AutoGenerated ,err error){
	mac := mac()
	pincode := strings.Split(mac,":")
	pinnum := make([]int, 6)
	if len(pincode) == 6 {
		for i, pin := range pincode {
	...
	Pinbuffer.WriteString("0" + strconv.Itoa(pinnum[0]%90+10))
	Pinbuffer.WriteString("-" + strconv.Itoa(pinnum[1]%90+10))
	Pinbuffer.WriteString("-" + strconv.Itoa(pinnum[2]+100))
	var sernum bytes.Buffer
	sernum.WriteString(strconv.Itoa(rand.Intn(100)))
	sernum.WriteString("." + strconv.Itoa(pinnum[3]%100))
	sernum.WriteString("." + strconv.Itoa(pinnum[4]%100))
	sernum.WriteString("." + strconv.Itoa(pinnum[5]%100))
