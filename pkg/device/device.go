...
//Envelope means the data transformed from coredata，后4个定义的结构体是coredata转化来的   具体用在控制上
...
//的alias没有重名的，相同alias的会在其后加((1)、(2)、(3)....等等表示)；2、homekit上的虚拟设备如果是调光灯在控制的时候就显示百分百，如果是开关灯
// 在控制的时候就显示开和关；3、指定homebridge的config.json文件的生成路径；
func Decode(jsonStr []byte, label string, statusport string) error {
	var projects []Project                    //这是project.go中定义的project的结构体
	err := json.Unmarshal(jsonStr, &projects) //json非序列化的返回
	if err != nil {
		return err
	}
	index := 1 // 看明白了  这是如果web上的zigbee设备的name相同则对应的虚拟设备的alias也相同，这是homekit在相同alias后面加上((1)、(2)、(3)....等等表示)以示区分
...
func createConfigData(accessaries []Accessary, statusport string) (configdata AutoGenerated, err error) {
	...
	username := fmt.Sprintf("%02X:%02X:%02X:%02X:%02X:%02X", pinnum[5], pinnum[4], pinnum[3], pinnum[2], pinnum[1], pinnum[0]) //获取本机的mac地址要反过来写的原因是如果正着写则第2次就访问不到homebridge了
